...
using TagshelfApi.Serialization.Concrete;
using TagshelfApi.Tooling.Abstract;

namespace TagshelfApi.Client.Abstract
{
    public class BaseClient : RestClient
    {
                          ...
                          string baseUrl)
        {
            ...
            UserAgent = $"{appName} - TagshelfApiClient";
        ...
        public T Get<T>(IRestRequest request) where T : new()
        {
            if (_apiKey != null)
            {
                request.AddHeader("X-TagshelfAPI-Key", _apiKey);
            }
